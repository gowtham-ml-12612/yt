...
import { TEAMS, uiColor } from './iplTeams.js';
...
function handleWallOrGap(ball, gap) {
  ...
  const nx = dx / (dist || 1);
  const ny = dy / (dist || 1);
  // Push inside with extra 1.5px clearance to prevent immediate re-trigger
  ball.x = ARENA.cx + nx * (ARENA.r - ball.r - 1.5);
  ball.y = ARENA.cy + ny * (ARENA.r - ball.r - 1.5);

  const vn = ball.vx * nx + ball.vy * ny;
  if (vn > 0) {
    const spd = Math.hypot(ball.vx, ball.vy) || ball.baseSpeed;
...
function drawBall(ctx, ball) {
  ...
  if (ball.imageEl?.complete && ball.imageEl.naturalWidth > 0) {
    ...
    ctx.clip();
    ctx.drawImage(ball.imageEl, x - ir, y - ir, ir * 2, ir * 2);
    ctx.restore();
  } else {
    ctx.save();
...
function TeamLine({ team }) {
  return (
    <span className="ball-inline-team">
      {team.imageSrc
        ? <img src={team.imageSrc} alt={team.name} className="ball-inline-team-mark ball-team-image" style={{ width: 20, height: 20, objectFit: 'contain' }} />
...
export default function RoyaleScreen({ matchConfig, onMatchEnd }) {
  ...
  function initState() {
    const balls = TEAMS.map((team) => {
      const ball = createBall(team);
      if (team.imageSrc) {
        const img = new Image();
        img.src = team.imageSrc;
        ball.imageEl = img;
      }
      return ball;
    });
      ...
      <div className="ball-game-layout ball-game-layout--royale">
        <div className="ball-record-frame ball-record-frame--royale" ref={recordFrameRef}>
          <div className="ball-royale-header">
            <div className="ball-royale-header-title">IPL 2026</div>
          ...
          <div className="ball-canvas-wrap ball-canvas-wrap--royale">
                  ...
                  <div className="ball-royale-target-grid">
                    {TEAMS.map((team) => {
                      ...
                      return (
                        ...
                        >
                          {team.imageSrc
                            ? <img src={team.imageSrc} alt={team.name} className="ball-royale-target-chip-flag" style={{ width: 22, height: 22, objectFit: 'contain' }} />
            ...
            {phase === 'countdown' && (
              <div className="ball-intro-overlay ball-intro-overlay--royale">
                <div className="ball-royale-title">IPL 2026</div>
              </div>
            )}
              ...
              <div className="ball-end-overlay">
                ...
                <div className="ball-end-flag-badge">
                  <div className="ball-end-flag" style={{ color: winnerTeam ? uiColor(winnerTeam) : '#ffffff' }}>
                    {winnerTeam
                      ? (winnerTeam.imageSrc
                        ? <img src={winnerTeam.imageSrc} alt={winnerTeam.name} style={{ width: 72, height: 72, objectFit: 'contain' }} />
                        : winnerTeam.flag)
                      : '⏱️'}
                  </div>
                </div>
        ...
        <aside className="ball-game-side">
          <div className="ball-side-card">
            ...
            <div className="ball-side-stage">All Teams Survival</div>
            <div className="ball-side-note">One moving border gap slowly grows. Any team that exits through that opening is eliminated.</div>
            <div className="ball-side-note ball-side-note--royale-target">
              {targetWinners.length ? `Target winners: ${targetWinners.map((team) => team.name).join(', ')}` : 'Target winners: off'}
            </div>
          </div>
