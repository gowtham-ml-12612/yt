// ---------------------------------------------------------------------------
// ballStorage.js  –  Tournament persistence via localStorage
// ---------------------------------------------------------------------------

const TOURNAMENT_KEY = 'ball_tournaments';
const ACTIVE_KEY = 'ball_active_tournament';
 ...
 * teams: array of team ids (4–8)
 */
export function createTournament(name, teamIds, format = 'league') {
  const id = `tourn-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;

  // Build standing entries for each team
  const standings = teamIds.map((teamId) => ({
    ...
    drawn: 0,
    lost: 0,
    scored: 0,   // power orb touches scored
    conceded: 0,
    points: 0,
  }));

  // Build fixture list using the circle/round-robin algorithm.
  // This interleaves matches by round so no single team plays many games in a row.
  // Each round has floor(n/2) matches; teams rotate around a fixed anchor.
  const fixtures = [];
  const n = teamIds.length;
  const teams = [...teamIds];
  // If odd number of teams, add a BYE placeholder so the algorithm stays even
  if (n % 2 !== 0) teams.push(null);
  const half = teams.length / 2;
  const numRounds = teams.length - 1;
  const rotating = teams.slice(1); // all except the fixed anchor at index 0

  for (let round = 0; round < numRounds; round++) {
    // Build this round's pairings
    ...
    // Rotate the non-anchor slice clockwise by 1
...
/**
 * Record a played fixture result.
 * Updates standings in the same tournament object.
 */
...
/** Next unplayed fixture in the active tournament */
